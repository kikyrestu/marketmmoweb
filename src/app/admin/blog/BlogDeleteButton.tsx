...
export default function BlogDeleteButton({ id }: { id: string }) {
  ...
  const handleDelete = async () => {
    ...
    if (res.ok) {
      setDeleted(true);
      import('sonner').then(({ toast }) => toast.success('Blog post deleted!'));
    } else {
      const msg = await res.json().then(j => j.message || 'Gagal hapus post!').catch(()=> 'Gagal hapus post!');
  ...
  return (
    <>
      <button className="underline text-red-600" onClick={() => setShowModal(true)} aria-label="Delete Blog">Delete</button>
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-xs w-full text-center">
            <h3 className="text-lg font-bold mb-4">Konfirmasi Hapus</h3>
            <p className="mb-4">Yakin mau hapus postingan ini?</p>
