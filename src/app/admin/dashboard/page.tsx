	...
	return (
		<div className="p-6 space-y-6">
			<div>
				...
				<p className="text-sm text-muted-foreground">Ringkasan metrik marketplace (fee {platformFeeBps/100}%).</p>
			</div>
			<form className="flex items-center gap-3">
				<label className="text-sm text-muted-foreground">Range:</label>
				<a className={`text-sm px-2 py-1 rounded border ${(!days || days===7)?'bg-accent':''}`} href="/admin/dashboard?days=7">7D</a>
				<a className={`text-sm px-2 py-1 rounded border ${days===30?'bg-accent':''}`} href="/admin/dashboard?days=30">30D</a>
				<a className={`text-sm px-2 py-1 rounded border ${days===90?'bg-accent':''}`} href="/admin/dashboard?days=90">90D</a>
			</form>
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div className="rounded-lg border p-4 bg-card">
					<div className="text-xs text-muted-foreground">GMV (Completed)</div>
					<div className="text-2xl font-semibold mt-1">{formatCurrency(gmvCompleted)}</div>
				</div>
				<div className="rounded-lg border p-4 bg-card">
					<div className="text-xs text-muted-foreground">Fee Revenue</div>
				...
				<div className="rounded-lg border p-4 bg-card">
							<h2 className="text-sm font-medium">Transactions by Status</h2>
					<div className="mt-3 grid grid-cols-2 gap-3 text-sm">
						...
						<div className="rounded-lg border p-4 bg-card">
							<h2 className="text-sm font-medium">GMV Trend</h2>
							<div className="mt-3">
								<div className="w-full h-36 relative">
									{/* simple sparkline w/o deps */}
									{(() => {
										if (!trend.length) return <div className="text-xs text-muted-foreground">No data</div>
								...
								<div className="mt-2 text-xs text-muted-foreground">{trend.length} points</div>
							</div>
						</div>
			</div>
					...
					<div className="grid gap-4 md:grid-cols-2">
						<div className="rounded-lg border p-4 bg-card">
							...
							<div className="mt-3 space-y-2">
								{topSellers.length === 0 && <div className="text-xs text-muted-foreground">No data</div>}
									...
									<div key={t.sellerId} className="flex items-center justify-between border rounded-md px-3 py-2">
										<div className="flex items-center gap-2">
											...
											<div className="flex flex-col">
												<span className="text-sm font-medium">{t.seller?.name || 'Unknown'}</span>
						...
						<div className="rounded-lg border p-4 bg-card">
							...
							<div className="mt-3 space-y-2">
								{recentTransactions.length === 0 && <div className="text-xs text-muted-foreground">No data</div>}
