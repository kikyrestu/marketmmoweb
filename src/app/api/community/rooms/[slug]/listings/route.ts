...
export async function GET(_req: NextRequest, ctx: any) {
  ...
  const room = await (prisma as any).communityRoom.findUnique({ where: { slug }, select: { id: true, isTradable: true } })
  if (!room) return NextResponse.json({ message: 'Room not found' }, { status: 404 })
  const listings = await (prisma as any).roomListing.findMany({
...
export async function POST(req: NextRequest, ctx: any) {
  ...
  const item = await (prisma as any).item.findUnique({ where: { id: itemId }, select: { id: true, sellerId: true, isAvailable: true } })
  ...
  const listing = await (prisma as any).roomListing.create({ data: { roomId: room.id, itemId: item.id, sellerId: session.user.id } })
...
export async function PATCH(req: NextRequest, ctx: any) {
  try {
    ...
    const listing = await (prisma as any).roomListing.findUnique({ where: { id }, select: { id: true, roomId: true, sellerId: true } })
    ...
    await (prisma as any).roomListing.update({ where: { id }, data: { status } })
